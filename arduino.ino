...
const unsigned long CONFIG_REFRESH_MS = 10UL * 60UL * 1000UL;
...
// Remote-configurable scheduling (defaults)
int measurementIntervalMinutes = MEASUREMENT_INTERVAL_MINUTES;
int startHourConfig = START_HOUR;
int endHourConfig = END_HOUR;
unsigned long lastConfigFetch = 0;
const char* intervalSource = "default";
...
int parseHourFromTime(const String &timeStr) {
  if (timeStr.length() < 2) return -1;
...
String extractString(const String &body, const char *key, const String &fallback) {
  String target = "\"" + String(key) + "\":\"";
  int idx = body.indexOf(target);
  if (idx == -1) return fallback;
  int start = idx + target.length();
...
void applyConfig(int freqMinutes, const String &startTime, const String &endTime) {
  if (freqMinutes > 0) {
    measurementIntervalMinutes = freqMinutes;
  }
  intervalSource = "remote";
  int sHour = parseHourFromTime(startTime);
  int eHour = parseHourFromTime(endTime);
  if (sHour >= 0 && sHour <= 23) startHourConfig = sHour;
  if (eHour >= 0 && eHour <= 23) endHourConfig = eHour;

  Serial.print("Applied config | freq(min)=");
  Serial.print(measurementIntervalMinutes);
  Serial.print(" | window=");
  Serial.print(twoDigit(startHourConfig));
  Serial.print(":00-");
  Serial.print(twoDigit(endHourConfig));
  Serial.print(":00");
  Serial.print(" | source=");
  Serial.println(intervalSource);
}

void fetchRemoteConfig() {
  if (WiFi.status() != WL_CONNECTED) return;
  const char* devId = deviceIds[currentDeviceIndex];
  String encodedDevId = String(devId);
  encodedDevId.replace(" ", "%20"); // URL-encode spaces in deviceId

  String path = String("/api/measurements/config?deviceId=") + encodedDevId;
  ...
  Serial.print("Config fetch status: ");
  Serial.println(statusCode);
  if (statusCode != 200 && statusCode != 201) {
    Serial.print("Config fetch body: ");
    Serial.println(body);
  }

  if (statusCode == 200 || statusCode == 201) {
    int freq = extractInt(body, "frequency", measurementIntervalMinutes);
    String start = extractString(body, "startTime", twoDigit(startHourConfig) + ":00");
    String end = extractString(body, "endTime", twoDigit(endHourConfig) + ":00");
    applyConfig(freq, start, end);
  } else {
    Serial.println("Use existing config due to fetch failure.");
  }
...
bool isTimeAllowed() {
  ...
  return (hour >= startHourConfig && hour < endHourConfig);
}
...
void syncTimeFromNTP() {
  ...
  for (int i = 0; i < 10; i++) {
    if (timeClient.update()) {
      ...
      Serial.print("NTP synchronization successfull epoch=");
...
void loop() {
  ...
  switch (currentState) {
    ...
    case STATE_IDLE: {
      if (WiFi.status() == WL_CONNECTED &&
          (lastConfigFetch == 0 || now - lastConfigFetch > CONFIG_REFRESH_MS)) {
        fetchRemoteConfig();
      }
      ...
      if (now - lastMeasurementTime >
          (unsigned long)measurementIntervalMinutes * 60000UL) {
          ...
          if (bufferIndex >= SPO2_BUFF_SIZE) {
            ...
            if (hr_valid && spo2_valid) {
              ...
              if (sendData(currentDevId, hr_val, spo2_val)) {
                Serial.println("Upload successfull");
